...
function YggDrasil() {
  ...
  useEffect(() => {
    document.title = "YGGDRASIL - HangryCoins";
  }, []);
  ...
  useEffect(() => {
    fetch(`https://hangrycoins.com:5001/homeGameProviderDetails`)
      ...
      .then((data) => {
        const yggDrasilDetails = data.filter(
          (item) => item.provider_name === "YGGDRASIL"
        );
        setDetails(yggDrasilDetails);
      })
      .catch((error) =>
  ...
  return (
    <div className="png-container">
      <div className="container">
        <div className="row">
          <h2 className="text-center mt-4">YGGDRASIL</h2>
          {details.map((detail) => (
